...
class Player():
    ...
    def update(self):
        ...
        if not self.dead:
            ...
            self.move()
            self.sprite = pygame.transform.rotate(self.MasterSprites[self.animationindex // ANIMATION_SPEED],
                                                  self.rotation)
    ...
    def handle_collisions(self):
        ...
        if self.x > SCREEN_SIZE[0] - self.radius or self.x < self.radius or self.y > SCREEN_SIZE[
            1] - self.radius or self.y < self.radius:
            self.speed = 0
    ...
    def draw(self):
        ...
        if not self.dead:
            ...
            self.x - self.sprite.get_width() / 2, self.y - 50, self.health * HEALTHBAR_SIZE[0], HEALTHBAR_SIZE[1]))
            if self.ammo1.cd_time > 0:
                pygame.draw.rect(self.screen, (150, 150, 150), (self.x - self.sprite.get_width() / 2, self.y - 52,
                                                                ((self.ammo1.cd_time + 0.0) / self.ammo1.cooldown) * 50,
                                                                2))
